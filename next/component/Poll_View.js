...
class Poll_View extends React.Component {
  state = {
    totalVote: this.props.poll.options
      .map((elem) => elem.vote)
  ...
  handleSubmitVote = (e) => {
    ...
    let voteIndex
    let votedOption
    ;[...document.querySelectorAll('[name="input-vote"]')].some(
      (vote, index) => {
        if (vote.checked) {
          voteIndex = index
    ...
    M.toast({
      html: `Congratulations, you've voted for ${
        votedOption ? '"' + votedOption + '"' : "something"
      }`,
      ...
      lastVotedOn = (
        <h6>
          ...
          <span
            ...
            data-position="top"
            data-tooltip={dateTooltipString(this.state.lastVotedOn)}
          >
            {" "}
            {relativeDate(this.state.lastVotedOn)}
          </span>
        </h6>
      )
    } else {
      firstVoteMessage = (
        <h5 className="center mb-3">Be the first to vote on this poll!!</h5>
      )
    }
    return (
      <div className="card grey lighten-5 z-depth-3 black-text">
        <div className="card-content">
                    ...
                    <li className="collection-item grey lighten-3" key={index}>
                      {!this.state.totalVote ? (
                        <div className="row my-0">
                          ...
                          <div className="col s3  right-align">{pct}</div>
                        </div>
                      ) : (
                        <div className="row my-0">
          ...
          <h6 className="right-align mr-5">
            Total Votes : {this.state.totalVote}
          </h6>
          {firstVoteMessage}
          ...
          <h6>
            ...
            <span
              className="tooltipped"
              data-position="top"
              data-tooltip={dateTooltipString(this.props.poll.createdOn)}
            >
              {" "}
              {relativeDate(this.props.poll.createdOn)}
            </span>
          </h6>
          {lastVotedOn}
        </div>
