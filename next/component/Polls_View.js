                                                ...
                                                <li className="collection-item grey lighten-3" key={index}>
                                                    {props.totalVote === 0 ?
                                                        ...
                                                        ${colorClasses[index % colorClasses.length]}`}
                                                                        style={{ width: pct.replace(/<|>/, "") }}></div>
                                                                </div>
                                                            </div>
                                                            <div className="col s2"><h6>{pct}</h6></div>
                                                        </div>
                                                    }
                                                </li>)
                                        }
    ...
    return (
        <div>
            {props.polls.map((poll, index) => {
                const totalVote = poll.options.map(elem => elem.vote).reduce((acc, curr) => acc + curr);
                return (
                    <Poll_View_Individual
                    key = {index}
                    poll = {poll}
                    totalVote = {totalVote}
                    />
