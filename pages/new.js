...
class New extends React.Component {
    ...
    handleAddMoreOptions = (e) => {
        ...
        const options = this.state.options;
        if (options + 1 === 32) {
            M.toast({ html: 'Maximum number of additional options reached!', classes: 'yellow black-text', displayLength: 6000 })
        }
        this.setState({ options: options + 1 });
        M.CharacterCounter.init(document.querySelectorAll(`textarea#input-option-${divId}`));
    }
                    ...
                    <div className="card-content">
                        <form className="mt-32">
                            <div className="input-field">
                                <textarea className="materialize-textarea" id="input-title" placeholder="Title for the pole" data-length="512"></textarea>
                                <label htmlFor="input-title" className="active">Title</label>
                            </div>
                            <div className="input-field">
                                <textarea className="materialize-textarea" name="input-option" data-length="256"></textarea>
                            ...
                            <div id="more-options" className="mb-2">
                            </div>
                            <button className="btn waves-effect waves-light yellow black-text accent-4"
                                disabled={this.state.options < 32 ? false : true}
                                onClick={e => this.handleAddMoreOptions(e)}>
                                <i className="material-icons left">add</i>
                                Add More Options
                        {this.state.options < 32 ? ` (${32 - this.state.options} left)` : null}
                            </button>
                            <br />
                            <br />
                            <button className="btn-large waves-effect waves-light yellow black-text accent-4" type="submit" id="btn-submit"
